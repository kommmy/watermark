...
/// 全 App 共用的深色设计 token。所有 View 都通过这里的常量来取色/字号/圆角，
/// 不要硬编码。改主题只需改这一个文件。
enum AppTheme {
    ...
    enum Palette {
        ...
        /// 选中态 / 高亮卡片
        static let elevated   = Color(hex: 0x3A3A3F)
    ...
    enum Shadow {
        static let card = (color: Color.black.opacity(0.35), radius: CGFloat(16), x: CGFloat(0), y: CGFloat(8))
        static let elevated = (color: Color.black.opacity(0.5), radius: CGFloat(24), x: CGFloat(0), y: CGFloat(12))
    }
}
